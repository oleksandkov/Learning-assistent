...
void AlgorithmComparator::printTable(const LevelComparisonReport& report,
    ...
    if (ansiSupported) {
        out << "\033[1;36m====================================================================================================\033[0m\n";
        out << "\033[90m* Search(ms) — чистий час пошуку steady_clock (без завантаження, UI, валідації).\n"
            << "* Replay — перевірка розв'язку симуляцією через GameRules (ReplayValidator).\n"
            << "* Deadlock Detection активний у строго безпечному режимі (Safe mode).\033[0m\n\n";
    } else {
        out << "====================================================================================================\n";
        out << "* Search(ms) - чистий час пошуку steady_clock (без завантаження, UI, валідації).\n"
            << "* Replay - перевірка розв'язку симуляцією через GameRules (ReplayValidator).\n"
            << "* Deadlock Detection активний у строго безпечному режимі (Safe mode).\n\n";
    }
}
